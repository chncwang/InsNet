...
public:
    ...
    void forward(Graph &cg, const std::vector<Node *>& x) {
        if (x.empty()) {
...
Node *add(Graph &graph, const std::vector<Node*> &inputs) {
    ...
    package_private::PAddNode *result = package_private::PAddNode::newNode(dim);
    result->forward(graph, inputs);
    return result;
}

BatchedNode *addInBatch(Graph &graph, const std::vector<BatchedNode *> &inputs) {
    package_private::BatchedPAddNode *node = new package_private::BatchedPAddNode;
