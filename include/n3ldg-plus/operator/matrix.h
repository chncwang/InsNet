...
public:
    explicit MatrixNode(const std::string &node_type) : Node(node_type + "-matrix") {}
};
...
public:
    ...
    void connect(Graph &graph, const std::vector<Node *> &inputs) {
        setInputs(inputs);
        for (Node *in : inputs) {
            in->addParent(this);
        }
    ...
    void connect(Graph &graph, NodeAbs &topo_input, const std::vector<Node *> &inputs) {
        setInputs(inputs);
    ...
    void setInputs(const std::vector<Node *> &inputs) override {
        int input_dim = inputs.front()->getDim();
        for (auto it = inputs.begin() + 1; it != inputs.end(); ++it) {
            if (input_dim != (*it)->getDim()) {
                std::cerr << "MatrixConcatNode - forward inconsistent input dims" << std::endl;
                abort();
            }
        }
    ...
    std::string typeSignature() const override {
        return "MatrixConcatNode-" + std::to_string(getRow());
    }
...
public:
    void init(Graph &graph, BatchedNode &input, int group) {
        if (input.batch().size() % group != 0) {
            std::cerr << fmt::format("input batch size:{} group:{}\n", input.batch().size(),
                    group);
        }
...
public:
    ...
    void init(Graph &graph, BatchedNode &matrix, BatchedNode &vec, int *dim = nullptr) {
        ...
        if (vec.batch().size() % group != 0) {
            std::cerr << fmt::format("BatchedTranMatrixMulVectorNode init vec size:{} group:{}\n",
...
public:
    ...
    void setInputs(const std::vector<Node *> &ins) override {
        ...
        if (a_size % k_ != 0) {
            std::cerr << fmt::format("MatrixMulMatrixNode setInputs a_size:{} k:{}\n", a_size, k_);
            abort();
        }
        ins_.at(0) = ins.at(0);
        ins_.at(1) = ins.at(1);
    ...
    std::string typeSignature() const override {
        return Node::getNodeType() + std::to_string(ins_.at(0)->getDim() / k_);
    }
...
public:
    ...
    void setInputs(const std::vector<Node *> &ins) override {
        ...
        if (getDim() > max_col) {
            std::cerr << fmt::format("TranMatrixMulVectorNode setInputs dim:{} max_col:{}\n",
                getDim(), max_col);
            abort();
        }
    }
    ...
    std::string typeSignature() const override {
        return Node::getNodeType() + std::to_string(vector_->getDim());
    }
...
public:
    ...
    void compute() override {
        ...
        if (use_lower_triangle_mask_) {
            if (a_col_ != b_col_) {
                std::cerr << fmt::format("a_col_:{} b_col_:{}\n", a_col_, b_col_);
                abort();
            }
    ...
    std::string typeSignature() const override {
        return Node::getNodeType() + std::to_string(input_row_) +
...
public:
    void init(Graph &graph, BatchedNode &a, BatchedNode &b, int input_row,
        ...
        if (use_lower_triangle_mask && a_col != b_col) {
            std::cerr << fmt::format("BatchedTranMatrixMulMatrixNode init a_col:{} b_col:{}\n",
                a_col, b_col);
            abort();
        }
...
Node *matrixAndVectorMulti(Graph &graph, Node &matrix, Node &vec) {
    ...
    if (matrix.getDim() % vec.getDim() != 0) {
        std::cerr << fmt::format("vec dim:%1% matrix dim:%2%", vec.getDim(), matrix.getDim()) <<
            std::endl;
        abort();
    }
...
BatchedNode *matrixAndVectorMulti(Graph &graph, Node &matrix, BatchedNode &vec,
        int *dim = nullptr) {
    if (matrix.getDim() % vec.getDim() != 0) {
        std::cerr << fmt::format("vec dim:{} matrix dim:{}\n", vec.getDim(), matrix.getDim());
        abort();
    }
