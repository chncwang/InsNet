...
public:
    ...
    void init(int out_dim, int in_dim, bool use_b = true,
        ...
        if (W_ != nullptr) {
            std::cerr << "UniParams init already initialized" << std::endl;
            abort();
        }
...
public:
    ...
    void setParam(BiasParam &param) {
        ...
        if (getDim() > bias_param_->outDim()) {
            std::cerr << fmt::format("dim is {}, but bias param dim is {}\n", getDim(),
                bias_param_->outDim());
            abort();
        }
    }
...
Node *linear(Graph &graph, Node &input, LinearParam &params) {
    if (input.getDim() % params.W().outDim() != 0) {
        std::cerr << fmt::format("linear input dim:{} W col:{} W col:{}\n", input.getDim(),
...
Node *linear(Graph &graph, Node &input, Param &param) {
    if (input.getDim() % param.outDim() != 0) {
        std::cerr << fmt::format("linear input dim:%1% W col:%2% W col:%3%\n", input.getDim(),
            input.getColumn(), param.inDim());
        abort();
    }

    static std::map<void *, LinearParam *> param_map;
    ...
    if (it == param_map.end()) {
        ...
        param_map.insert(std::make_pair(&param, uni_params));
    } else {
