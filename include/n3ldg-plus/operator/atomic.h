...
public:
    ...
    void compute() override {
        val().vec() = getInput().val().vec().unaryExpr(std::ptr_fun(ftanh));
    }
...
public:
    ...
    void compute() override {
        val().vec() = getInput().val().vec().unaryExpr(std::ptr_fun(fsigmoid));
    }

    void backward() override {
        getInput().loss().vec() += loss().vec() * getInput().val().vec().binaryExpr(val().vec(),
                std::ptr_fun(dsigmoid));
    }
...
public:
    ...
    void compute() override {
        val().vec() = getInput().val().vec().unaryExpr(std::ptr_fun(frelu));
    }
...
public:
    ...
    void compute() override {
        val().vec() = getInput().val().vec().unaryExpr(std::ptr_fun(fsqrt));
    }

    void backward() override {
        getInput().loss().vec() += loss().vec() * val().vec().unaryExpr(std::ptr_fun(dsqrt));
    }
...
public:
    ...
    std::string typeSignature() const override {
        return Node::getNodeType() + "-" + std::to_string(drop_value_);
    }
...
public:
    ...
    std::string typeSignature() const override {
        return getNodeType() + std::to_string(getInput().getDim());
    }
...
public:
    ...
    void init(Graph &graph, BatchedNode &input, const std::vector<int> &rows) {
        std::vector<int> dims(rows.size());
        int i = 0;
        for (int row : rows) {
...
public:
    void init(Graph &graph, BatchedNode &input, const std::vector<dtype> &factors) {
...
Node *maxScalar(Graph &graph, Node &input, int input_col) {
    package_private::MaxScalarNode *node = package_private::MaxScalarNode::newNode(input_col);
    node->forward(graph, input);
    return node;
}

BatchedNode *maxScalar(Graph &graph, BatchedNode &input, int input_col) {
    package_private::BatchedMaxScalarNode *node = new package_private::BatchedMaxScalarNode;
    node->init(graph, input, input_col);
    return node;
};

Node *tanh(Graph &graph, Node &input) {
    package_private::TanhNode *result = package_private::TanhNode::newNode(input.getDim());
    result->forward(graph, input);
    return result;
}

Node *sigmoid(Graph &graph, Node &input) {
    package_private::SigmoidNode *result = package_private::SigmoidNode::newNode(input.getDim());
    result->forward(graph, input);
    return result;
}

Node *relu(Graph &graph, Node &input) {
    using namespace package_private;
...
BatchedNode *scaled(Graph &graph, BatchedNode &input, dtype factor) {
    using namespace package_private;
    std::vector<dtype> factors(input.batch().size());
