...
dtype softMaxLoss(Node * x, const vector<dtype> &answer, Metric& eval,
    dtype batchsize) {
    int nDim = x->getDim();
...
#if USE_GPU
dtype softMaxLoss(const std::vector<Node *> &x, const std::vector<int> &answers,
...
dtype cost(Node * x, const vector<dtype> &answer, int batchsize = 1) {
    int nDim = x->getDim();
    int labelsize = answer.size();
    if (labelsize != nDim) {
        std::cerr << "softmax_loss error: dim size invalid" << std::endl;
