...
#include <memory>
#include "fmt/core.h"
#include "insnet/cuda/helper_cuda.h"
#include "insnet/cuda/print.cu"
#include "insnet/cuda/print.cuh"
#include "insnet/cuda/memory_pool.h"
#include "insnet/base/tensor.h"
#include "insnet/base/memory.h"
#include "insnet/util/profiler.h"

using insnet::cuda::MemoryPool;
using std::vector;
using std::string;
using std::map;
...
__global__ void KernelMax(dtype **v, int count, int dim, int *max_indexes, dtype *max_vals) {
    ...
    for (int i = threadIdx.x; i < dim; i += blockDim.x) {
        dtype x = v[count_i][i];
        if (x > shared_max[threadIdx.x]) {
            shared_max[threadIdx.x] = x;
            shared_max_i[threadIdx.x] = i;
        }
    }
    __syncthreads();
...
void Max(dtype **v, int count, int dim, int *max_indexes, dtype *max_vals) {
    ...
    dim3 block_dim(count, 1, 1);

    KernelMax<<<block_dim, thread_count>>>(v, count, dim, max_indexes, max_vals);
    CheckCudaError();
#if TEST_CUDA
...
__global__ void KernelMaxScalarForward(dtype **v, int count, int head_count, int* head_dims,
        ...
        int *block_counters,
        int *max_indexes,
        dtype **max_vals) { // TODO need to be optimized
    __shared__ volatile dtype shared_max[TPB];
    __shared__ volatile int shared_max_i[TPB];
    __shared__ volatile bool is_last_block;
...
__global__ void KernelVectorSumForward(dtype **v, int count, int col, int *rows,
        ...
        int *block_counters,
        dtype **results) { // TODO need to be optimized
    __shared__ volatile extern dtype shared_sum[];
    __shared__ volatile bool is_last_block;
    if (threadIdx.x == 0 && blockIdx.y == 0) {
        block_counters[blockIdx.x] = 0;
...
__global__ void KernelVectorSum(dtype **v, int count, int *rows, int *cols, dtype *results) {
    __shared__ volatile extern dtype shared_sum[];
    ...
    for (int i = threadIdx.x; i < row; i += blockDim.x) {
        shared_sum[threadIdx.x] += v[count_i][row * col_i+ i];
    }
    __syncthreads();
    ...
    if (threadIdx.x == 0) {
        results[count_i * gridDim.y + blockIdx.y] = shared_sum[0];
    }
}
...
__global__ void KernelVectorSum(dtype *v, int count, int *rows, int max_row, int *cols,
    ...
    for (int i = threadIdx.x; i < row; i += blockDim.x) {
        ...
        shared_sum[threadIdx.x] += v[v_offset + offset];
    }
    __syncthreads();
    for (int i = (blockDim.x >> 1); i > 0; i >>= 1) {
        if (threadIdx.x < i) {
...
void SoftmaxBackward(vector<dtype *> &grads, dtype **vals, int count, int *rows, int max_row,
    ...
    KernelVectorSum<<<block_dim, thread_count, thread_count * sizeof(dtype)>>>(a.value, count,
            rows, max_row, cols, offsets, z.value);
    CheckCudaError();
...
__global__ void KernelExp(dtype **in_vals, dtype *subtrahends, int count, int row, int *cols,
        int max_col,
        dtype **vals) {
    int i = DeviceDefaultIndex();
    int n = row * max_col;
...
void LogSoftmaxForward(dtype **in_vals, int count, int row, int *cols, int max_col,
    ...
    int block_count = DefaultBlockCountWithoutLimit(count * row * max_col);
    KernelExp<<<block_count, TPB>>>(in_vals, max_val_arr.value, count, row, cols, max_col, vals);
    CheckCudaError();
...
__global__ void KernelLogSoftmaxBackward(dtype **grads, int count, int row, int *cols,
        int max_col,
        dtype *z,
        dtype **vals,
        dtype **in_grads) {
...
void LogSoftmaxBackward(dtype **grads, dtype **vals, int count, int row, int *cols, int max_col,
    ...
    KernelLogSoftmaxBackward<<<block_count, TPB>>>(grads, count, row, cols, max_col, z_arr.value,
            vals, in_grads);
    CheckCudaError();
}
