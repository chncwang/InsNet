...
__global__ void KernelTranMatrixMulVectorForward(dtype **matrices, dtype **vectors, int count,
        int *cols,
        int max_col,
        int row,
        dtype *block_sums,
    ...
    shared_sum[threadIdx.x] = row_i < row ? matrices[count_i][offset] * vectors[count_i][row_i] :
        0.0f;
    __syncthreads();
...
void TranMatrixMulVectorForward(vector<dtype *> &matrices, vector<dtype *> &vectors, int count,
    ...
    NumberPointerArray matrix_val_arr, vector_val_arr, val_arr;
    matrix_val_arr.init((dtype**)matrices.data(), matrices.size());
    vector_val_arr.init((dtype**)vectors.data(), vectors.size());
    val_arr.init((dtype **)vals.data(), vals.size());

    IntArray col_arr;
    col_arr.init(cols.data(), cols.size());

    KernelTranMatrixMulVectorForward<<<block_dim, thread_count, thread_count * sizeof(dtype)>>>(
            matrix_val_arr.value, vector_val_arr.value, count, col_arr.value, max_col, row,
...
__global__ void KernelTranMatrixMulMatrixForward(dtype **a, dtype **b, int *a_cols, int *b_cols,
        int row,
        bool use_lower_triangle_mask,
        dtype **vals) {
    __shared__ volatile extern dtype shared_sum[];

    int a_col = a_cols[blockIdx.x];
    int b_col = use_lower_triangle_mask ? a_col : b_cols[blockIdx.x];
    if (blockIdx.y >= a_col || blockIdx.z >= b_col) {
        return;
    }

    if (use_lower_triangle_mask && blockIdx.y > blockIdx.z) {
        if (threadIdx.x == 0) {
            vals[blockIdx.x][a_col * blockIdx.z + blockIdx.y] = -INF;
        }
        return;
    }
    ...
    if (threadIdx.x == 0) {
        vals[blockIdx.x][a_col * blockIdx.z + blockIdx.y] = shared_sum[0];
    }
}
...
void TranMatrixMulMatrixForward(vector<dtype *> &input_a_vals, vector <dtype *> &input_b_vals,
    ...
    dim3 block_dim(count, max_a_col, max_b_col);
    KernelTranMatrixMulMatrixForward<<<block_dim, thread_count, thread_count * sizeof(dtype)>>>(
            a_val_arr.value, b_val_arr.value, a_col_arr.value, b_col_arr.value, row,
            use_lower_triangle_mask, val_arr.value);
    CheckCudaError();
}
...
__global__ void KernelTranMatrixMulMatrixBackward(dtype **grads, dtype **in_vals, int count,
    ...
    int other_col = other_cols[blockIdx.x];
    shared_sum[threadIdx.x] = threadIdx.x < other_col ?
        in_vals[blockIdx.x][row * threadIdx.x + blockIdx.y] *
        (trans ? grads[blockIdx.x][threadIdx.x * col + blockIdx.z] :
         grads[blockIdx.x][blockIdx.z * other_col + threadIdx.x]) : 0;
    __syncthreads();
...
__global__ void KernelMatrixMulMatrixForward(dtype **a, dtype **b, int *ks, int *b_cols, int row,
        dtype **vals) {
    __shared__ volatile extern dtype shared_sum[];
    int b_col = b_cols[blockIdx.x];
    if (blockIdx.z >= b_col) {
        return;
    }
    int k = ks[blockIdx.x];
...
__global__ void KernelMatrixMulMatrixBackwardForB(dtype **grads, dtype **a_vals, int *ks,
        int *b_cols,
        int row,
        dtype **b_grads) {
    __shared__ volatile extern dtype shared_sum[];
    ...
    int k = ks[blockIdx.x];
    int b_col = b_cols[blockIdx.x];
    if (blockIdx.y >= b_col || blockIdx.z >= k) {
        return;
    }
    ...
    if (threadIdx.x == 0) {
        DeviceAtomicAdd(b_grads[blockIdx.x] + k * blockIdx.y + blockIdx.z, shared_sum[0]);
    }
}
...
__global__ void KernelMatrixAndVectorMultiBackwardForVector(dtype **grads, dtype **matrices,
    ...
    int block_sums_offset = blockIdx.x * max_col * gridDim.y + blockIdx.z * gridDim.y + blockIdx.y;
    if (threadIdx.x == 0) {
...
void SoftmaxForward(vector<dtype *> &in_vals, int count, vector<int> &rows, vector<int> &cols,
    ...
    IntArray row_arr, col_arr;
    row_arr.init(rows.data(), count);
    col_arr.init(cols.data(), count);
//    cout << boost::format("SoftmaxForward count:%1% max_row:%2% max_col:%3%") % count % max_row %
//        max_col << endl;
    KernelMaxScalar<<<block_dim, thread_count, thread_count * sizeof(dtype)>>>(
            (dtype **)input_arr.value, count, row_arr.value, col_arr.value, block_maxes.value,
            block_counters.value, max_val_arr.value);
    CheckCudaError();
#if TEST_CUDA
//    vector<dtype *> cpu_in_vals(count);
//    vector<dtype> cpu_max_scalars;
//    for (int i = 0; i < count; ++i) {
//        int dim = rows.at(i) * cols.at(i);
//        cpu_in_vals.at(i) = new dtype[dim];
//        CallCuda(cudaMemcpy(cpu_in_vals.at(i), in_vals.at(i), dim * sizeof(dtype),
//                    cudaMemcpyDeviceToHost));
//        for (int col_i = 0; col_i < cols.at(i); ++col_i) {
//            dtype max = Mat(cpu_in_vals.at(i) + rows.at(i) * col_i, rows.at(i), 1).maxCoeff();
//            cpu_max_scalars.push_back(max);
//        }
//        for (int j = 0; j < max_col - cols.at(i); ++j) cpu_max_scalars.push_back(0.0f);
//    }
//    vector<dtype> copied_max_scalars(count * max_col);
//    CallCuda(cudaMemcpy(copied_max_scalars.data(), max_val_arr.value,
//                count * max_col * sizeof(dtype), cudaMemcpyDeviceToHost));
//    for (int i = 0; i < count; ++i) {
//        for (int col_i = 0; col_i < cols.at(i); ++col_i) {
//            dtype cpu_v = cpu_max_scalars.at(i * max_col + col_i);
//            dtype gpu_v = copied_max_scalars.at(i * max_col + col_i);
//            cerr << "gpu:" << endl;
//            printVector(copied_max_scalars);
//            cerr << "cpu:" << endl;
//            printVector(cpu_max_scalars);
//        }
//    }
#endif
    ...
    NumberPointerArray val_arr;
    val_arr.init((dtype**)vals.data(), vals.size());
    KernelExp<<<block_count, TPB>>>(input_arr.value, max_val_arr.value, count, row_arr.value,
            max_row, col_arr.value, max_col, val_arr.value);
    CheckCudaError();

    KernelVectorSum<<<block_dim, thread_count, thread_count * sizeof(dtype)>>>(
            (dtype **)val_arr.value, count, row_arr.value, col_arr.value, block_maxes.value,
            block_counters.value, max_val_arr.value);
    CheckCudaError();

    KernelDiv<<<block_count, TPB>>>(val_arr.value, max_val_arr.value, count, row_arr.value,
            max_row, col_arr.value, max_col, val_arr.value);
    CheckCudaError();
#if TEST_CUDA
//    for (int i = 0; i < count; ++i) {
//        delete []cpu_in_vals.at(i);
//    }
#endif
}
...
__global__ void KernelPointwiseMul(dtype **a, dtype **b, int count, int *rows, int max_row,
    ...
    for (int i = index; i < count * max_row * max_col; i += step) {
        ...
        if (col_i < col) {
            int row_i = offset % row;
            if (row_i < row) {
                vals[count_i * n + offset] = a[count_i][offset] * b[count_i][offset];
            }
        }
    }
}
...
__global__ void KernelVectorSum(dtype *v, int count, int *rows, int max_row, int *cols,
    ...
    int row_i = blockIdx.y * blockDim.x + threadIdx.x;
    int offset = col_i * row + row_i;
    shared_sum[threadIdx.x] = row_i < row ? v[count_i * gridDim.z * max_row + offset] : 0.0f;
    __syncthreads();
...
__global__ void KernelSoftmaxBackward(dtype **vals, dtype **grads, int count, int *rows,
    ...
    for (int i = index; i < count * max_row * max_col; i += step) {
        ...
        if (col_i < col) {
            int row_i = offset % row;
            if (row_i < row) {
                dtype b = z[count_i * max_col + col_i] - a[count_i * n + offset];
...
void SoftmaxBackward(vector<dtype *> &grads, vector<dtype *> &vals, int count, vector<int> &rows,
    ...
    NumberPointerArray grad_arr, val_arr;
    grad_arr.init(grads.data(), count);
    val_arr.init(vals.data(), count);
    IntArray row_arr, col_arr;
    row_arr.init(rows.data(), count);
    col_arr.init(cols.data(), count);
    int block_count = DefaultBlockCount(max_col * max_row * count);
    KernelPointwiseMul<<<block_count, TPB>>>(grad_arr.value, val_arr.value, count, row_arr.value,
            max_row, col_arr.value, max_col, a.value);
    CheckCudaError();
    ...
    KernelVectorSum<<<block_dim, thread_count, thread_count * sizeof(dtype)>>>(a.value, count,
            row_arr.value, max_row, col_arr.value, block_vals.value, block_counters.value,
            z.value);
    CheckCudaError();

    NumberPointerArray in_grad_arr;
    in_grad_arr.init(in_grads.data(), count);
    KernelSoftmaxBackward<<<block_count, TPB>>>(val_arr.value, grad_arr.value, count,
            row_arr.value, max_row, col_arr.value, max_col, z.value, a.value, in_grad_arr.value);
}
...
__global__ void KernelPointwiseLinearForward(dtype **in_vals, int count, int dim, dtype *g,
    ...
    for (int i = index; i < count * dim; i += step) {
        int count_i = i / dim;
        int dim_i = i % dim;
        vals[count_i][dim_i] = g[dim_i] * in_vals[count_i][dim_i] + b[dim_i];
    }
}
...
void PointwiseLinearForward(vector<dtype *> &in_vals, int count, int dim, dtype *g, dtype *b,
    ...
    val_arr.init(vals.data(), count);
    int block_count = DefaultBlockCount(count * dim);
    KernelPointwiseLinearForward<<<block_count, TPB>>>(in_val_arr.value, count, dim, g, b,
            val_arr.value);
    CheckCudaError();
}

__global__ void KernelPointwiseLinearBackwardForG(dtype **grads, dtype **in_vals, int count,
        int dim,
        dtype *g_grads,
        int *block_counters,
        dtype *block_sums) {
    ...
    int dim_i = blockIdx.x;
    int count_i = blockIdx.y * blockDim.x + threadIdx.x;
    shared_sum[threadIdx.x] = count_i < count ?
        grads[count_i][dim_i] * in_vals[count_i][dim_i] : 0.0f;
    __syncthreads();
    ...
    if (is_last_block) {
        ...
        if (threadIdx.x == 0) {
            dtype x = shared_sum[0];
            DeviceAtomicAdd(g_grads + dim_i, x);
        }
    }
}
...
__global__ void KernelPointwiseLinearBackwardForInput(dtype **grads, dtype *g_vals, int count,
    ...
    for (int i = index; i < count * dim; i += step) {
        int count_i = i / dim;
        int dim_i = i % dim;
        DeviceAtomicAdd(in_grads[count_i] + dim_i, grads[count_i][dim_i] * g_vals[dim_i]);
    }
}
...
void PointwiseLinearBackward(vector<dtype *> &grads, vector<dtype *> &in_vals, dtype *g_vals,
        ...
        dtype *g_grads,
        dtype *bias_grads) {
    NumberPointerArray grad_arr, in_val_arr, in_grad_arr;
    grad_arr.init(grads.data(), count);
    in_val_arr.init(in_vals.data(), count);
    in_grad_arr.init(in_grads.data(), count);

    int thread_count = min(NextTwoIntegerPowerNumber(count), TPB);
    int block_y_count = (count - 1 + thread_count) / thread_count;
    dim3 block_dim(dim, block_y_count, 1);
    NumberArray block_sums;
    block_sums.init(block_y_count * dim);
    IntArray block_counters;
    block_counters.init(dim);

    KernelPointwiseLinearBackwardForG<<<block_dim, thread_count, thread_count * sizeof(dtype)>>>(
            grad_arr.value, in_val_arr.value, count, dim, g_grads, block_counters.value,
            block_sums.value);
    CheckCudaError();

    KernelPointwiseLinearBackwardForBias<<<block_dim, thread_count,
        thread_count * sizeof(dtype)>>>(grad_arr.value, count, dim, bias_grads,
                block_counters.value, block_sums.value);
    CheckCudaError();

    int block_count = DefaultBlockCount(count * dim);
    KernelPointwiseLinearBackwardForInput<<<block_count, TPB>>>(grad_arr.value, g_vals, count, dim,
            in_grad_arr.value);
    CheckCudaError();
}
